...
class PaymentExternalSystemAdapterImpl(
    ...
    override fun performPaymentAsync(paymentId: UUID, amount: Int, paymentStartedAt: Long, deadline: Long) {
        ...
        if (
            ...
            paymentESService.update(paymentId) {
                it.logProcessing(
                    ...
                    now(),
                    transactionId,
                    reason = "Apparently there is not enough time to complete request"
                )
            }
            return
        }
        ...
        try {
            ...
            for (i in 0..<retryCnt) {
                if (success) {
                    logger.info("Payment successfully processed for txId: $transactionId, payment: $paymentId in " +
                            "${Duration.ofMillis(now().minus(curTime))}")
                    break
                }
                ...
                val responseConsumer = fun(response : Response) {
                    ...
                    logger.warn("[$accountName] Payment processed for txId: $transactionId, payment: $paymentId, succeeded: ${responseBody.result}, message: ${responseBody.message}, code: ${response.code}")
                    if (response.code == 429 && !responseBody.result) {
